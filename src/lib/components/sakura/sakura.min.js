...
var Sakura = function (t, e) {
	...
	(this.createPetal = function () {
		...
		var t = [
			...
			a = d(t),
			n =
				(0.007 * document.documentElement.clientHeight + Math.round(5 * Math.random())) *
				c.settings.fallSpeed,
			i = [
				'fall '.concat(n, 's linear 0s 1'),
				''.concat(e, ' ').concat((30 < n ? n : 30) - 20 + m(0, 20), 's linear 0s infinite'),
		...
		(o.style.background = 'linear-gradient('
			...
			(o.style.borderRadius = ''
				.concat(m(c.settings.maxSize, c.settings.maxSize + Math.floor(10 * Math.random())), 'px ')
				.concat(m(1, Math.floor(s / 4)), 'px')),
			(o.style.height = ''.concat(r, 'px')),
			(o.style.left = ''.concat(Math.random() * document.documentElement.clientWidth - 100, 'px')),
...
(Sakura.prototype.start = function () {
	...
	(Sakura.prototype.stop = function () {
		...
		a && (window.cancelAnimationFrame(a), this.el.setAttribute('data-sakura-anim-id', '')),
			t ||
				setTimeout(function () {
					for (var t = document.getElementsByClassName(e.settings.className); 0 < t.length; )
